...
export default {
  ...
  methods: {
    async submit() {
      // Frontend validation
      if (!this.form.name.trim())        return this.show('Name is required', 'error')
      if (!this.form.description.trim()) return this.show('Description is required', 'error')
      if (!this.form.price || this.form.price <= 0) return this.show('Enter a valid price', 'error')
      if (!this.form.image_url.trim())   return this.show('Image URL is required', 'error')
...
.form-card { background: white; border-radius: 12px; padding: 36px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); }
